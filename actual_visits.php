...
$query = "SELECT visits.id, visits.idService,
            visits.dateVisit, visits.hourVisit, visits.payInAdvance, visits.idDoctor, visits.clientQR,
            ...
            WHERE visits.idUser=:id ORDER BY visits.dateVisit, visits.hourVisit";
...
{ 
    ...
    exit();
        ...
        $response["success"] = 1;
        $response["message"] = "Displayed actual visits"  ;
			  ...
			  foreach ($result as $row)
				{
                    ...
                    if($checkDate >= time()) //tutaj stare sa kasowane
                    {
						...
						$index_qr= "token_" .strval($i);
                        $response[$index_qr] = $row->clientQR;
